...
namespace tinySTL {
	...
	template<typename ForwardInterator>
	inline void __destory_aux(ForwardInterator first, ForwoardInterator last, __true_type) {}


	template<typename ForwardInterator>
	inline void __destory_aux(ForwardInterator first, ForwoardInterator last, __false_type) {
		while (first < last) {
			destory(&*first);
		}
	}
